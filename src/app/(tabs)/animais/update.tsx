...
export default function UpdateAnimal() {
  ...
  useEffect(() => {
    ...
    const fetchAnimalData = async () => {
      try {
        setLoading(true);
        const res = await api.get<AnimalData>(`/animals/${animalId}`);
        if (res.status !== 200) throw new Error("Erro na resposta da API");
        const data = res.data;
  ...
  const handleUpdate = async () => {
    ...
    try {
      const res = await api.patch(`/animals/${animalId}`, payload);

      if (res.status !== 200) {
        console.error("Resposta de erro:", res.data);
  ...
  const handleInputChange = (
    ...
    if (!animal) return;
    setAnimal(
      (prev) => ({ ...(prev as AnimalData), [field]: value } as AnimalData)
    );
  };
    ...
    >
      ...
      {error ? (
        <View className="bg-red-50 border border-red-200 p-4 rounded-lg mb-6">
          ...
          <Button
            onPress={() =>
              router.replace(`/animais/update?animalId=${animalId}`)
            }
        ...
        <View className="space-y-6">
          ...
          <View>
            ...
            <TextInput
              ...
              value={animal.maeId ?? ""}
              onChangeText={(t) =>
                handleInputChange("maeId", t === "" ? null : t)
              }
